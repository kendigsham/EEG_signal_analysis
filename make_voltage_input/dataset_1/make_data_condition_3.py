...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 25 10:01:05 2024
...
for temp_file in edited_file_list:
    ...
    for color_name, color_index in event_id.items():
        ...
        no_eye_channels = temp_data[:,occipital_channels,:]   ## only the ocipital channels
    
        print(no_eye_channels.shape)   ## numpy  (trials, 6, 256)
        ...
        for trial_index in range(shape_time[0]):
            ...
            mean_channel = np.mean(one_trial_data, axis=0)   ### shape (51,)
    
            
            for index in range(mean_channel.shape[0]):
                dict_for_channel_mean[f'time_{index}'].append(mean_channel[index])
...
save_dir = 'condition_3__data'

df_combine = pd.concat(dataframe_list)
    
df_combine.to_csv(f'{save_dir}/df_combined_condition_3__mean_channel_all_times.csv')
