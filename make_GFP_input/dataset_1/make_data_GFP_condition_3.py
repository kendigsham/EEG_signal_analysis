...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug  6 10:02:59 2024
...
for temp_file in edited_file_list:
    ...
    for color_name, color_index in event_id.items():
        ...
        no_eye_channels = temp_data[:,:64,:]   ## only the ocipital channels
        ...
        for trial_index in range(shape_time[0]):
            ...
            GFP_time_series = np.std(one_trial_data, axis=0)   ### shape (51,)   ##### this is the GFP
    
            
            for index in range(GFP_time_series.shape[0]):
                dict_for_channel_mean[f'time_{index}'].append(GFP_time_series[index])
...
save_dir = 'condition_3__GFP_data'

df_combine = pd.concat(dataframe_list)
    
df_combine.to_csv(f'{save_dir}/df_combined_condition_3__GFP_all_times.csv')
